...
To build and run the Triangle sample for emscripten on OSX or Linux:
...
On **Windows**, replace the emscripten-make-release config with **emscripten-ninja-release**
(Oryol comes with a precompiled ninja tool on Windows, since make doesn't work
for some reason).
...
First setup the emscripten SDK, careful, this will overwrite the global ~/.emscripten config file:
...
This uses the official emscripten SDK (http://kripken.github.io/emscripten-site/docs/getting_started/downloads.html),
it is recommended to update or modify the emscripten SDK directly using the **emsdk** tool after the
initial setup (under _sdks/[platform]/emsdk_portable_).

On Windows, the make-tool is currently not supported (if you already have make in the path, you can try it out,
but I was getting errors when compiling). So instead of the emscripten-make-release config, use the
**emscripten-ninja-release** to compile Oryol:

```
> cd ~/oryol
> oryol select emscripten-ninja-release
> oryol build
```

This works because there's a precompiled ninja.exe tool under _tools/win32_.

Cross-compiling in Visual Studio using the emscripten VS plugin is currently not supported.
