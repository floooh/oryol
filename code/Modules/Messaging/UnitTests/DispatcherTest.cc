...
TEST(DispatcherTest) {
    ...
    Ptr<Broadcaster> sink = Broadcaster::Create(StringAtom("sink"));
    Ptr<Dispatcher<TestProtocol>> disp0 = Dispatcher<TestProtocol>::Create(StringAtom("disp0"));
    Ptr<Dispatcher<TestProtocol2>> disp1 = Dispatcher<TestProtocol2>::Create(StringAtom("disp1"));
