...
TEST(OpenModeTest) {
    ...
    CHECK(OpenMode::Invalid == 5);  // this will fail if we need to add more modes
    ...
    CHECK(String(OpenMode::ToString(OpenMode::Invalid)) == "Invalid");
    ...
    CHECK(OpenMode::FromString("Invalid") == OpenMode::Invalid);
}
