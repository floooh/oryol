...
TEST(MeshFactoryTest) {
    ...
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().NumComponents() == 2);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().ByteSize() == 20);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().ComponentByteOffset(0) == 0);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().ComponentByteOffset(1) == 12);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().Component(0).Valid());
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().Component(0).Attr() == VertexAttr::Position);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().Component(0).Format() == VertexFormat::Float3);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().Component(0).ByteSize() == 12);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().Component(1).Valid());
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().Component(1).Attr() == VertexAttr::TexCoord0);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().Component(1).Format() == VertexFormat::Float2);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().Component(1).ByteSize() == 8);
    ...
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().NumComponents() == 0);
    CHECK(mesh.GetVertexBufferAttrs().GetVertexLayout().ByteSize() == 0);
