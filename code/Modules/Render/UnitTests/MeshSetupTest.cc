...
TEST(MeshSetupTest) {
    ...
    MeshSetup s1 = MeshSetup::FromFile("s1", 1, Usage::Stream, Usage::Dynamic);
    CHECK(s1.ShouldSetupFromFile());
    CHECK(!s1.ShouldSetupFromData());
    CHECK(!s1.ShouldSetupEmpty());
    CHECK(s1.GetLocator() == "s1");
    CHECK(s1.GetVertexUsage() == Usage::Stream);
    CHECK(s1.GetIndexUsage() == Usage::Dynamic);
    ...
    CHECK(s2.GetVertexUsage() == Usage::Stream);
    CHECK(s2.GetIndexUsage() == Usage::Dynamic);
    ...
    MeshSetup s4 = MeshSetup::FromData("s4", Usage::Stream, Usage::Dynamic);
    CHECK(!s4.ShouldSetupFromFile());
    CHECK(!s4.ShouldSetupEmpty());
    CHECK(s4.ShouldSetupFromData());
    CHECK(s4.GetLocator() == "s4");
    CHECK(s4.GetVertexUsage() == Usage::Stream);
    CHECK(s4.GetIndexUsage() == Usage::Dynamic);
    ...
    CHECK(s5.GetVertexUsage() == Usage::Stream);
    CHECK(s5.GetIndexUsage() == Usage::Dynamic);
    ...
    CHECK(s6.GetVertexUsage() == Usage::Stream);
    ...
    CHECK(s7.GetVertexUsage() == Usage::Dynamic);
    CHECK(s7.GetIndexUsage() == Usage::Stream);
